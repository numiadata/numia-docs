...
With your API key in hand, you can now start making API requests. Here’s how to make a simple test request using `curl`:

<Tabs>
  <TabItem value="lcd" label="Osmosis LCD" default>
    ```bash
    curl https://osmosis-lcd.numia.xyz/blocks/latest   
    -H "Accept: application/json"   
    -H "Authorization: Bearer <Token>"
    ```
  </TabItem>
  <TabItem value="rpc" label="Osmosis RPC">
    ```bash
    curl https://osmosis-rpc.numia.xyz/blocks/latest   
    -H "Accept: application/json"   
    -H "Authorization: Bearer <Token>"
    ```
  </TabItem>
...
<Card 
  title="Osmosis"
  link="/reference/osmosis/core-concepts#osmosis"
...
<Card 
  title="Quasar"
  link="/reference/quasar/core-concepts#quasar"
  icon="arrow-square-up-right"
  iconType="regular"
  iconSize="32px"
/>
