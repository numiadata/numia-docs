...
Here’s the revised version with a bulleted approach for the parameters:
...
- **`position_id` (Query Parameter)**: 
  - The unique identifier for the ad placement, such as `wallet_home`.

- **`bearer_token` (Header)**:
  - The authentication token required to authorize the request.
