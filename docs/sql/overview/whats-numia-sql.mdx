...
Currently, Numia provides SQL-based access to on-chain data through BigQuery, so you can query billions of rows using SQL-like syntax, making it an ideal solution for working with Numia’s extensive datasets.

<Image 
  src="/media/sql/sql-overview-2.png"
/>
...
<Image 
  src="/media/sql/sql-overview-1.png"
/>

<Callout type="info" title="Note">
Google charges based on the amount of data processed by each query, regardless of computation power. Numia’s raw data tables can be terabytes in size, but we regularly create smaller, optimized tables to reduce query costs. For tips on minimizing expenses, refer to the [Optimze Queries](/sql/querying-data/optimize-queries) page.
</Callout>